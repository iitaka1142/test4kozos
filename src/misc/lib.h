...
/** メモリ上のデータの比較を行う */
comp_result memcmp(const void * b1, const void * b2, long len);
/**<
 * \param b1  比較するデータ1への先頭ポインタ
 * \param b2  比較するデータ2への先頭ポインタ
 * \param len 領域のサイズ
 *
 * \retval Same    2つは同じ
 * \retval Later   b1が大きい
 * \retval Earlier b2が大きい
 */
...
/**<
 * \param[in] s1 比較する文字列1への先頭ポインタ
 * \param[in] s2 比較する文字列2への先頭ポインタ
 *
 * \retval Same    2つは同じ
 * \retval Later   s1が大きい
 * \retval Earlier s2が大きい
 */
