...
#define CONSDRV_DEVICE_NUM 1 //コマンド送受信のデバイス数
#define CONS_BUFFER_SIZE 24

struct consreg {
  serial_port port; // 4byte
...
{
  serial_send_byte(cons->port, cons->send_buf[0]);
  cons->send_len--;
...
/// コマンドから指定されたシリアルポートを抜き出す
static inline unsigned int get_drv_index(char buf[])
{
...
static int send_string(struct consreg * cons, char * str, unsigned int len)
{
  ...
  if ((cons->send_len > 0) && (serial_intr_is_send_enable(cons->port) == DISABLE)) {
    // 送信割り込み無効の場合:
    send_char(cons); // 最初の1文字を送信(残りは送信有効割り込みのハンドラで送信)
    serial_intr_send_enable(cons->port); //   送信を開始する
  }
  else {
    // 送信割り込み有効の場合:
    //   なにもしない
...
{ 
  if ( (cons->sender == (kz_thread_id_t)NULL) || (cons->send_len == 0) ) {
    ...
    serial_intr_send_disable(cons->port);
  } else {
    send_char(cons);
  }
...
{
  char c = serial_recv_byte(cons->port);
...
static void consdrv_intr(void)
{
  ...
  for( int i = 0; i < CONSDRV_DEVICE_NUM; i++ ) {
    cons = &(consreg[i]);
    if ( cons->sender != (kz_thread_id_t)NULL ) {
      if ( serial_is_send_enable(cons->port) == ENABLE ) {
	(void)consdrv_send_intrproc(cons);
      }
      if ( serial_is_recv_enable(cons->port) == ENABLE ) {
 ...
 static int consdrv_command(struct consreg * cons, kz_thread_id_t id, unsigned int size, char * command)
{
  ...
  case CONSDRV_CMD_USE:
    cons->port = get_serial_port_from((port_index)(command[1] - '0'));
    cons->sender = id;
    ...
    serial_init(cons->port);
    serial_intr_recv_enable(cons->port);
    break;
